...
2. 소스가 진실입니다. 노드와 소스가 다르면 소스를 따르고 노드를 고칩니다.
...
summary: 클래스·생성자·static·상속·인터페이스·다형성·접근지정자. StarCraft/LoL/동물 테마 예제 다수.   # (필수) 120자 이내 한 줄. ROOT 표는 이 줄만 인용
...
선택 섹션(필요할 때만 추가): `## 복습 체크리스트`, `## 퀴즈 시드`, `## 복습 기록`, `## 근거 공백(이 리포에 없는 것)`.
...
`## 배경` / `## 전제` / `## 안건`(안건마다 현황 · 선택지와 장단점 · 지식맵 영향) / `## 소유자에게 묻는 질문` / `## 결정 기록`(표: 날짜 | 안건 | 결정 | 근거) / `## 후속 작업`.

## 8. Claude 읽기 프로토콜

1. **세션 시작**: CLAUDE.md만 자동 로드됩니다. 여기서 얻는 것은 리포 목적, 절대 규칙, 지식맵 위치와 로드 조건뿐입니다.
2. **ROOT.md를 읽는 조건**: 이 리포의 자바 개념 질문("상속 예제 어디 있었죠?", "소켓 채팅은 몇 단계였죠?"), 특정 패키지 질문("study_190318_mk3이 뭐였죠?"), 학습 이력·타임라인, 복습 요청, 리포 정리 논의.
   **읽지 않는 조건**: git 명령·브랜치 작업, `.gitignore`/LICENSE/README 편집, 지식맵과 무관한 파일 조작, 이 리포와 무관한 일반 자바 질문("Stream API 사용법 알려 주세요").
...
4. **매칭 실패 폴백**: (a) 패키지명·테마 이름("스타크래프트", "라면")이 보이면 `grep -n "<키워드>" knowledge-map/INDEX.md`로 행을 찾아 주 단원을 엽니다. (b) 그래도 없으면 소유자에게 되묻습니다. 어떤 경우에도 `units/`·`src/` 전체 나열로 대체하지 않습니다.
5. **패키지명 질문**: unit을 열기 전에 INDEX.md의 해당 행만 읽습니다. 행의 한 줄 요약으로 답이 되면 거기서 멈춥니다.
...
10. **예산**: CLAUDE.md + ROOT + unit 1~2개 + INDEX 행 몇 줄 ≈ 3~4k 토큰이 지식맵의 최대 부담입니다. 이 안에서 답이 안 나오면 소스를 직접 읽는 쪽으로 전환합니다.
