...
summary: legacy/ 패키지 36개를 날짜순으로 주 단원·부 단원·한 줄 요약·주의와 함께 나열한 색인.
stack: java
parent: ./ROOT.md
---
...
- 실제 경로는 `legacy/<주 단원 디렉토리>/<패키지>/`입니다(예: 주 단원 04 → `legacy/04-io-network-threads/study_190318/`). 경로가 바뀌면 이 문장과 각 단원의 `evidence`만 갱신합니다.
- `변환` 열: `-`는 아직 Markdown으로 변환하지 않은 패키지, 링크는 변환된 topic 문서입니다([GUIDE 10절](./GUIDE.md)).
...
| 패키지 | 날짜 | 종류 | 주 | 부 | 한 줄 요약 | 주의 | 변환 |
|---|---|---|---|---|---|---|---|
| study_181217 | 2018-12-17 | study | 01 | - | Hello World. 가장 이른 패키지 | - | - |
| study_181218 | 2018-12-18 | study | 01 | - | 기본 타입·변수·산술/비교 연산자·묵시적 형변환·while·Scanner | Var1.java 한글 깨짐('占쏙옙', 복구 불가), 나머지 MS949 | - |
...
| study_190304 | 2019-03-04 | study | 05 | - | Oracle 샘플 스키마 emp 테이블 JDBC SELECT/INSERT. PreparedStatement/ResultSet, finally 자원 해제 | ojdbc6.jar 삭제됨 | - |
...
| study_190306_room2 | 2019-03-06 | study | 02 | - | 다른 패키지에서 default 메서드 접근 불가 확인, 객체 println(Object 기본 toString) | room1의 MySite 의존. 실패 라인 주석 처리 | - |
...
| study_190308_mk3 | 2019-03-08 | study | 05 | - | Coolsms(nurigo) SDK + json-simple로 SMS 발송 | SDK jar 삭제됨 | - |
| study_190308_mk4 | 2019-03-08 | study | 02 | 03 | Object.toString 오버라이딩·StringBuilder·parseInt·hashCode | - | - |
| study_190311 | 2019-03-11 | study | 04 | - | 표준 입력/파일 입출력 스트림(바이트·문자·버퍼)·FileWriter/PrintWriter·URLConnection 웹 저장 (7 java) | D:/workspace 절대 경로 | - |
...
| study_190313 | 2019-03-13 | study | 05 | 04 | 뉴스 100건 URLConnection+Jsoup 파싱 → Oracle NEWS 저장 미니 크롤러(DAO/VO 원형) | 한 파일 5클래스. UTF-8 | - |
...
| study_190318_mk2 | 2019-03-18 | study | 04 | - | 2단계: Runnable 내부 클래스 + 스레드로 양방향 1:1 채팅 | - | - |
...
| study_190328 | 2019-03-28 | study | 05 | - | Class.forName/DriverManager/Statement/ResultSet. book·customer 조회, Scanner LIKE 검색 | 소문자 클래스명(booklist, name). 문자열 연결 LIKE | - |
